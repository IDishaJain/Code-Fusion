...
//         else {
//             const user = new User({ username, email, password, cpassword, role });
//             const savedUser = await user.save(); // Save the user and get the result
//             if (savedUser) {
//                 res.status(201).json({ success: "User Register Successfully" }); // Use 201 for successful creation
//                 console.log("User saved successfully:", savedUser); // Log the saved user data
//             } else {
//                 res.status(500).json({ error: "Failed to save user" });
//                 console.error("Error during user save: User data was not saved.");
//             }
//         }
//     } catch (err) {
//         console.error("Registration Error:", err); // Log the full error
//         res.status(500).json({ error: "Failed to register user" }); // Send an error response
//     }
